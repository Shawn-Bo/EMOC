...
def main(args):
    ...
    # 加载测试数据
    eval_dataset, eval_source_samples = EmotionClassifierDatesetGenerator().generate_dataset_from_file(
        project_path("benchmarks/test.json"),
        device,
        tokenizer,
        label2id,
        demo_dataset=args.demo_dataset,
    ...
    for epoch in range(train_epoch):
        ...
        #     """
        model.eval()
        for (input_ids, token_type_ids, attention_mask), source_sample in tqdm(
                zip(eval_dataloader, eval_source_samples)):
            with torch.no_grad():
                outputs = model(
                    input_ids=input_ids,
                    token_type_ids=token_type_ids,
            ...
            LABELS.append(int(predicted_label[0]))
            IDS.append(source_sample[0])
        ...
        with open("1190301419-樊宣伯.csv", "w") as f:
            for id, sentence_label in zip(IDS, LABELS):
                f.write(f"{id},{sentence_label}\n")
        exit()
